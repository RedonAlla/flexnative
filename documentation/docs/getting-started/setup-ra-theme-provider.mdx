...
**FlexNative ThemeProvider** from *FlexNative* is a component that makes the theme available throughout your app.
...
It accepts a [value prop](#themeproviderprops) which is the data that will be shared with all components that are consumers of that context.
...
| -- | -- | -- | -- | -- |
| colors | [`BaseTheme<TColors>`](#themecontextprops) | ***true*** | null | Where **TColors** is the object with custom colors. |
| colorScheme  | [`ColorSchemeName`](https://reactnative.dev/docs/usecolorscheme#supported-color-schemes) | **false** | null | Value indicates the current user preferred color scheme. |
